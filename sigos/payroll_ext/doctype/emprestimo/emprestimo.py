...
class Emprestimo(Document):
	...
	def _validar_mes_nao_passado(self):
		"""Reject a start date already past (e.g. Março quando estamos em Junho, or a
		manually-typed past date). The Dez→Jan wrap in ano_para_mes keeps year-end
		mes_referencia selections in the future, so only genuine back-references are
		blocked. Edit-safe: only fires for new records or when the start date is
		actually changed — an existing loan already mid-repayment isn't retroactively
