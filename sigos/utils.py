...
def calcular_faltas_detalhado(vigilante: str, start_date, end_date) -> list:
	...
	for a in absences:
		d = getdate(a.data)
		if a.regime and a.turno:
			# Real regime/turno context — Normal rows always have it; Falta de
			# Reserva rows do too when the guard has a real Reserva-escala
			# assignment for that day (metodo_faltas_reserva == "Peso do Turno").
			# Always derive the BASE from the regime — never from the stored
			# n_de_faltas, which is itself the de-dup'd value (else we'd de-dup
			# twice) — and apply the same consecutive de-dup either way.
			...
			# No regime/turno context — Abandono de Posto always (flat by
			# design), or Falta de Reserva with no escala match (Valor Fixo
			# mode, or Peso do Turno but no Reserva escala covers this guard's
			# delegação). The weight was already fully resolved and stamped at
			# Ausencias-save time (a flat Settings constant) — trust it
			# directly, there's no real schedule to de-dup against.
